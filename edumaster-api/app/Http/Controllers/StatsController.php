...
final class StatsController extends Controller
{
	public function getTotals(Request $request)
	{
		...
		if ($request->user()->role === 'student' || ($request->has('student_id') && $request->user()->role === 'teacher')) {
			...
			$totalInProgressWorksheets = EloquentWorksheetStudent::where('status', 'in_progress')->where(
				'student_id',
				$userId
			)->count();
			$totalCompletedWorksheets = EloquentWorksheetStudent::where('status', 'completed')->where(
				'student_id',
				$userId
			)->count();
		} else {
